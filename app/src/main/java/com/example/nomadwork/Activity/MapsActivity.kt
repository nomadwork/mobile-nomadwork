...
import com.example.nomadwork.API.request.WorkStationRequest
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchEdit = findViewById(R.id.simpleSearchView)



        val buttonWSRegister = findViewById<FloatingActionButton1>(R.id.button_ws_register)

        buttonWSRegister.setOnClickListener {
            val fragment = NewWorkStationFragment()

            val t = supportFragmentManager.beginTransaction()
            t.setCustomAnimations(R.anim.enter_from_right,
                R.anim.exit_to_left)
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        mMap.setOnMarkerClickListener(GoogleMap.OnMarkerClickListener {
            if(WorkStationHelper.checkInitialized()){
                val mark = WorkStationHelper.getWSByPosition(it.position.latitude, it.position.longitude)
        ...
        mMap.setOnMapClickListener {
            /*System.out.println(supportFragmentManager.findFragmentByTag("details"))
            if(supportFragmentManager.fragments.isNotEmpty() && supportFragmentManager.findFragmentByTag("details") != null){
                val fragment = supportFragmentManager.findFragmentByTag("details")
                val f = supportFragmentManager.beginTransaction()
                f.setCustomAnimations(R.anim.enter_from_bottom,
                    R.anim.exit_to_bottom)
                f.remove(fragment!!).commit()
            }*/
        ...
        mMap.setOnCameraMoveStartedListener {
            ...
            if(LocationHelper.distanceCurrentLocationAndNewLocation(LocationHelper.getUserLocation()!!.latitude,
                ...
                //mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(myLocation, 16.0f))
                getMarkersByLocation(mMap.projection.fromScreenLocation(Point()).latitude, mMap.projection.fromScreenLocation(Point()).longitude) {
                    addMarkersByLocation()
                }
